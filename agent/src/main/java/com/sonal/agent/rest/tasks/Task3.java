...
public class Task3 implements ITask {
	...
	public Mono<TaskResultVO> execute(JobRequestVO jobRequestVO) {
		...
		try {
			...
			process = Runtime.getRuntime().exec("/usr/local/bin/consul --version");
			waitFor = process.waitFor();
		} catch (IOException | InterruptedException e) {
			e.printStackTrace();
			taskResult = new TaskResultVO(jobRequestVO.getJobRequestId(),"Task3","Task3 Failed To Execute", false);
			result = Mono.just(taskResult);
		}
		
		if(waitFor == 0){
			taskResult = new TaskResultVO(jobRequestVO.getJobRequestId(),"Task3","Task3 Success Fully Executed", true);
			result = Mono.just(taskResult);
		}else{
			taskResult = new TaskResultVO(jobRequestVO.getJobRequestId(),"Task3","Task3 Failed To Execute", false);
			result = Mono.just(taskResult);
